...
/// Chunks in, one graph out. Mutations rebuild the dense arrays; reads are
/// plain slice access.
...
impl RoadGraph {
    ...
    /// Copy a decoded chunk into the graph and rebuild. Decoding stays
    /// zero-copy; this is where the caller's buffer stops being needed.
    ...
    /// Drop a chunk and rebuild. Returns whether it was loaded.
    ...
    pub fn index_of(&self, id: NodeId) -> Option<u32> {
        self.index_of_node.get(&id).copied()
    }
    ...
    /// Edges with at least one non-resident endpoint, in dense-index order.
    pub fn boundary_edges(&self) -> impl Iterator<Item = &GraphEdge> {
        self.edges.iter().filter(|e| e.boundary)
    }

    pub fn boundary_edge_count(&self) -> usize {
        self.edges.iter().filter(|e| e.boundary).count()
    }

    /// Deterministic rebuild of the dense arrays from the loaded chunks.
    ///
    /// Nodes are deduplicated by global id, preferring the resident record;
    /// edges are deduplicated by global id, so a border edge written into both
    /// of its chunks appears exactly once.
    fn rebuild(&mut self) {
        ...
        let mut seen_edges: HashMap<EdgeId, ()> = HashMap::with_capacity(self.edges.capacity());
        for build in self.chunks.values() {
            for e in &build.edges {
                if seen_edges.insert(e.gid, ()).is_some() {
                    continue;
                }
